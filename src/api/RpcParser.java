...
public class RpcParser {
	...
	public static void writeOutput(HttpServletResponse response, JSONObject obj) {
		try {			
			response.setContentType("application/json"); // use json to transfer data
			response.addHeader("Access-Control-Allow-Origin", "*"); // right to write and read
			PrintWriter out = response.getWriter(); // 'out' is the container to store values
			out.print(obj);
			out.flush();
			out.close();
		} catch (Exception e) {
